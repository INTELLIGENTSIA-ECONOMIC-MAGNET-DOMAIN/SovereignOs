...
export class IdentityManager {
    ...
    normalizeMember(raw) {
        // Ensure identity objects always have expected internal model structure
        return {
            officialName: raw.officialName || 'Unknown',
            sovereignName: raw.sovereignName || raw.name || 'Unknown',
            userName: raw.userName || raw.user_name || 'Unknown',
            gender: raw.gender || 'Unknown',
            country: raw.country || 'Unknown',
            membershipNo: raw.membershipNo || raw.membership_no || 'Unknown',
            boundMachineId: raw.boundMachineId || 'Unknown',
            profileAvatar: raw.profileAvatar || null,
            identityState: raw.identityState || raw.identity_state || 'Unknown',
            registrationState: raw.registrationState || raw.registration_state || 'Unknown',
            isFrozen: raw.isFrozen || raw.is_frozen || false,
            rank: raw.rank || { name: 'Unknown', code: null },
            membership: raw.membership || { status: 'Unknown', category: 'Unknown', startDate: null, endDate: null },
            security: {
                uid: raw.security?.uid || raw.id || 'unknown',
                abbr: raw.security?.abbr || raw.rank?.code || 'Unknown',
                rank: raw.security?.rank || raw.rank?.name || 'Unknown',
                ipBinding: raw.security?.ipBinding || null,
                clearance: raw.security?.clearance || 1,
                since: raw.security?.since || 'Unknown',
                authorizationKey: raw.security?.authorizationKey || 'LEGACY_ALLOTMENT',
                deviceFingerprint: raw.security?.deviceFingerprint || 'CORE_WORKSTATION'
            },
            location: raw.location || { country: raw.country || 'Unknown' },
            demographics: raw.demographics || { gender: raw.gender || 'Unknown', dob: 'Unknown' },
            specialRecognition: Array.isArray(raw.specialRecognition) ? raw.specialRecognition : (raw.special_recognitions || []),
            
            // Media properties (for dossier view)
            media: {
                profile: raw.media?.profile || raw.profileAvatar || 'https://via.placeholder.com/300x280?text=PROFILE',
                idFront: raw.media?.idFront || 'https://via.placeholder.com/150x150?text=ID+FRONT',
                idBack: raw.media?.idBack || 'https://via.placeholder.com/150x150?text=ID+BACK'
            },
            
            // Contact properties (for dossier view)
            contact: {
                phone: raw.contact?.phone || 'Unknown',
                email: raw.contact?.email || 'Unknown'
            },
            
            // Additional dossier display properties
            position: raw.position || 'Sovereign',
            titles: Array.isArray(raw.titles) ? raw.titles : (raw.titles ? [raw.titles] : ['Sovereign']),
            documentId: raw.documentId || 'DOC-000000',
            joinedThealcohesion: raw.joinedThealcohesion || raw.membership?.startDate || 'Unknown',
            joinedAC: raw.joinedAC || 'Unknown',
            joinedTLC: raw.joinedTLC || 'Unknown',
            remarks: raw.remarks || 'No remarks',
            actionCenter: raw.actionCenter || raw.country || 'Unknown',
            tlca: raw.tlca || raw.membershipNo || raw.membership_no || 'Unknown',
            rankDate: raw.rankDate || raw.membership?.startDate || 'Unknown',
            awards: Array.isArray(raw.awards) ? raw.awards : [],
            
            // Flat properties for compatibility
            name: raw.sovereignName || raw.name || 'Unknown',
            status: raw.membership?.status || 'Unknown',
            clearance: raw.membership?.category || 'Unknown',
            bio: 'Unknown',
            avatar: 'Unknown',
            specialties: Array.isArray(raw.specialRecognition) ? raw.specialRecognition : (raw.special_recognitions || []),
            certifications: [],
            missions: [],
            achievements: [],
            notes: 'Unknown'
    ...
    try {
        ...
        // Add validation to ensure data is an array and normalize each member
        this.members = Array.isArray(data) ? data.map(m => this.normalizeMember(m)) : [];
        this.isLoading = false;
        this.selectedMember = null; // reset selection on fresh sync
        this.render();
    } catch (error) {
...
renderDirectory() {
    // Create filtered members with their original indices
    const filteredWithIndices = this.members
        .map((member, originalIndex) => ({ member, originalIndex }))
        .filter(({ member: m }) => {
            const query = this.searchQuery.toLowerCase();
            const sovereignName = (m.sovereignName || '').toLowerCase();
            const uid = (m.security?.uid || '').toLowerCase();
            const country = m.location?.country || 'UNKNOWN';
            const gender = m.demographics?.gender || 'UNKNOWN';

            const matchSearch = sovereignName.includes(query) || uid.includes(query) || 
                              (m.userName || '').toLowerCase().includes(query) || 
                              (m.country || '').toLowerCase().includes(query) ||
                              (m.officialName || '').toLowerCase().includes(query) ||
                              (m.membershipNo || '').toLowerCase().includes(query);
            const matchRank = this.filters.rank === "ALL" || m.security?.abbr === this.filters.rank;
            const matchGender = this.filters.gender === "ALL" || (m.gender || '').toUpperCase() === this.filters.gender;
            ...
            const matchStatus = this.filters.status === "ALL" || 
                           (this.filters.status === "FROZEN" && m.isFrozen === true) ||
                           (this.filters.status === "PENDING" && m.identityState === "PROSPECT") ||
                           (this.filters.status === "ACTIVE" && m.identityState === "VERIFIED");
        ...
        <div class="registry-view" style="padding: 40px;">
            <div class="registry-header" style="border-left: 4px solid var(--id-gold); padding-left: 20px; margin-bottom: 40px;">
                ...
                <div style="font-size: 9px; color: #444; letter-spacing: 2px; margin-top: 5px;">
                    ACTIVE_UPLINKS: ${filteredWithIndices.length} // TOTAL_DATABASE: ${this.members.length}
                </div>
            </div>
            ...
            <div style="margin-top:20px;">
                ${filteredWithIndices.map(({ member: m, originalIndex }) => {
                const isFrozen = m.isFrozen || false;
                return `
                    <div class="device-entry member-row" data-idx="${originalIndex}" 
                        ...
                        <div style="display:flex; align-items:center; gap: 20px;">
                            ${m.profileAvatar ? `<img src="${m.profileAvatar}" style="width:40px; height:40px; border-radius:50%; object-fit:cover; border:2px solid var(--id-gold);" alt="Avatar">` : '<div style="width:40px; height:40px; border-radius:50%; background: var(--id-gold); display:flex; align-items:center; justify-content:center; color:#000; font-weight:bold;">?</div>'}
                            <div style="flex:1;">
                                <div style="color:#fff; font-size:13px; font-weight:bold;">${m.officialName || 'Unknown'} ${isFrozen ? '[LOCKED]' : ''}</div>
                                <div style="color:#444; font-size:9px;">@${m.userName || 'Unknown'} | ${m.sovereignName || 'Unknown'}</div>
                                <div style="color:#666; font-size:8px; margin-top:2px;">
                                    ${m.gender || 'Unknown'} | ${m.country || 'Unknown'} | ${m.membershipNo || 'Unknown'}
                                </div>
                                <div style="color:#888; font-size:7px; margin-top:1px;">
                                    Machine: ${m.boundMachineId || 'Unknown'}
                                </div>
                            </div>
                        </div>
                        <div class="rank-tag">${m.rank?.name || 'Unknown'}</div>
                    </div>
                `;
            }).join('')}
            </div>
...
getNodeTelemetry(nodeId) {
    ...
    this.container.querySelectorAll('.member-row').forEach(row => {
        row.onclick = () => {
            ...
            this.selectedMember = this.members[idx]; // already normalized
            this.vaultUnlocked = false; 
            this.render();
        };
    });
